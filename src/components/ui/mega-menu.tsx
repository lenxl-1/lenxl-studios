        ...
        {items.map((navItem) => (
          ...
          >
            {navItem.link && !navItem.subMenus ? (
              <a
                href={navItem.link}
                className="relative flex cursor-pointer items-center justify-center gap-1 py-1.5 px-4 text-sm text-foreground/70 transition-colors duration-300 hover:text-foreground group rounded-full"
              ...
              >
                ...
                {isHover === navItem.id && (
                  <motion.div
                    layoutId="hover-bg"
                    className="absolute inset-0 size-full bg-foreground/10"
                    style={{
                      borderRadius: 99,
                    }}
                  />
            ...
            <AnimatePresence>
              {openMenu === navItem.label && navItem.subMenus && (
                <div className="absolute left-0 top-full w-auto pt-2 z-50">
                  <motion.div
                    className="w-max border border-border bg-card p-4 shadow-xl text-card-foreground"
                    style={{
                      borderRadius: 16,
                    }}
                    layoutId="menu"
